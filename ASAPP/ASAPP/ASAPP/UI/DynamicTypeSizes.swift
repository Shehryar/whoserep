...
enum TextStyle {
    ...
    private static let defaultSize = 16.0
    
    private static let sizeTable: [TextStyle: [TextSizeCategory : Double]] = [
        body : [
            .xSmall : 11
        ]
    ]
    
    static func pointSize(for textStyle: TextStyle, sizeCategory: TextSizeCategory) -> Double {
        if let styleSizeTable = sizeTable[textStyle],
            let size = styleSizeTable[sizeCategory] {
            return size
        }
        return defaultSize
    }
    
    static func pointSize(for textStyle: TextStyle, sizeCategory: UIContentSizeCategory) -> Double {
        return pointSize(for: textStyle, sizeCategory: TextSizeCategory.from(contentSizeCategory: sizeCategory))
    }
}
